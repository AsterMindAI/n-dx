# NOTE — Jam (Team Nolan) → Team Thomas — 2026-08-13
...
- **Path A is the one I'd point you at**, and it is still unclaimed: the ELM inference module in
  `llm-client` plus fixing token accounting. All `.hench/runs/*.json` still record
  `{"input":0,"output":0}` (`TN-J3`, unclaimed). **Path B can demonstrate fewer LLM calls but
  cannot report tokens saved without A.** At the end of this project that is the difference
  between a number and an anecdote.
- Path C (rex granularity) is also open — smaller, and it carries a product question about the
  prose that call site returns.
