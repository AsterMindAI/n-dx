...
| 2026-08-28 | **Jam** (absorbed from K2, 2026-09-04) | Nolan | **REASSIGNED 2026-09-04: K2 is retired (lead's decision) and its open rows — `TN-J25`, `TN-J30`, `TN-J31`, `TN-J32` — are now Jam's. Completed rows keep K2's name; retiring an agent does not rewrite who measured what. ⚠️ K2 had no charter and no roster row, so there is no session log of its reasoning to inherit — only the artifacts, `K2-HANDBOOK.md`, and the backlog.** `TN-J25` K1 replacement (ADR up) + `TN-J30` — commissioning gold set #2, FUNDED by the lead 2026-08-31.** Phases 1–2 of `TN-J23` done. Phase 3 now proceeding: **freeze the model first**, then build the packet. ⚠️ **This claim now covers real LLM spend and a write into another repo.** Gold set #2 needs fresh LLM-labelled files, and corpus #1 already harvested the entire LLM-labelled population of both existing repos — so a **new repo must be cloned and analyzed with the LLM classify pass enabled**. Per Butter's finding, `analyze <other-repo>` writes `.sourcevision/` into the **target** repo, which no worktree isolates; targets are staged under `~/n-dx-elm-corpus/` (never the session scratchpad, which gets reaped). | **Writes** `scripts/elm-freeze-model.mjs`, `scripts/elm-goldset2-*.mjs`, `scripts/data/elm-frozen-model.json`, the gold-set-#2 packet, and Team Nolan docs. **Runs `sourcevision analyze --full` against newly cloned repos under `~/n-dx-elm-corpus/`** — real LLM calls, bounded and reported. **Does not run any state-writing `ndx` command against this checkout.** No `packages/**`, no shared files. **EXTENDED 2026-09-04 (`TN-J32`):** Phase 3 failed on generalisation, so this claim now also covers the corpus rebuild — `scripts/elm-corpus-build.mjs`, `scripts/elm-architecture-sweep.mjs`, `scripts/data/elm-archetype-corpus-v2.json`, `scripts/data/elm-architecture-sweep-v2.json` and `Claude-Context/Nolan-Agents/ELM-CORPUS.md`. **13 further classify calls spent** (express 1, fastify 2, commerce 1, got 1, Vue core 8) on repos staged under `/Users/nolanmoore/Work/n-dx-elm-corpus/`. Still no `packages/**` and no shared files. | On the v2 coverage re-check returning a verdict — gold set #2 remains blind and unlabelled, so `TN-J30`'s labelling decision is not blocked by this |
...
| 2026-09-16 | Nutella | Nolan | `TN-N1`/`TN-N2` — **Team Nolan's assigned scope is the ELM training database** (leads' decision 2026-09-16), and this claims **corpus acquisition**: staging public repos, running the `sourcevision` classify pass to label files against the 17-archetype catalog, harvesting rows, the seeded split, class coverage and teacher provenance. New agent, charter created today. **This is a scope claim as much as a file claim** — corpus acquisition was previously inside Jam's `TN-J*` rows, and `TN-J32`/`TN-J9` are corpus rows still on Jam's claim. **I am not taking those**; the board is the lock and Jam's claim predates mine. Filed as `TN-N3` for the lead. ⚠️ **Correction to this board:** § 2 still says the corpus-v2 re-freeze is *RUNNING (`pid 25842`)*. It is **finished** — `ps -p 25842` returns nothing and `scripts/data/elm-frozen-model-v2.json` exists (12,933 bytes, mtime 2026-09-04 11:58), which is that job's own stated success test. Left for the lead to amend, since § 2 is lead-maintained. | Writes `Claude-Context/Nolan-Agents/{Nutella.md,README.md,BACKLOG.md}`, `Claude-Context/Nolan-Agents/Notes/NOTE-nolan-internal-2026-09-16-*.md`, and this row. **Going forward under `TN-N2`:** `scripts/data/elm-archetype-corpus*.json` and `scripts/elm-corpus-build.mjs` (announced per the shared-`scripts/` rule), and `Claude-Context/Nolan-Agents/ELM-CORPUS.md`. **No `packages/**`. No shared files** — no `package.json`, no `pnpm-lock.yaml`, no `tests/e2e/**`, no `Claude-Context/` root docs. A separate-repo deliverable would add no workspace dependency. **State-writing:** `sourcevision analyze <repo> --full` writes `.sourcevision/` into the **target** repo under `/Users/nolanmoore/Work/n-dx-elm-corpus/`, not this checkout; if I ever need to analyze **this** repo I claim it here first. I am on the **shared checkout** `/Users/nolanmoore/Work/n-dx-1`, branch `Nolan-Work`, alongside Jam, Fluff and Syrup (lead's decision 2026-09-16). **Adding `scripts/elm-classify-residue.mjs`** (announced before creation per the shared-`scripts/` rule): sends ONLY a repo's still-unclassified files through sourcevision's real `enrichClassificationsWithLLM`, with the teacher resolved from the target repo's own `.n-dx.json` exactly as `analyze.ts:107-108` does. Exists because `--only=classifications` skips the inventory phase, so `changedFiles` is undefined and `classify.ts:100`'s reuse branch never fires — every file is re-sent, including labels already paid for. Backs up `classifications.json` before writing. Imports `packages/*/dist` directly (read-only use of the documented escape hatch); edits no package. ✅ **RELEASED 2026-09-21 21:10Z — harvest complete (typeorm, nest, remix); nothing is analyzing the staging tree.** ~~CLAIMING THE STAGING TREE 2026-09-21 for the class-targeted harvest~~ (`TN-N20`, ADR accepted by the lead). Running `sourcevision analyze` against `/Users/nolanmoore/Work/n-dx-elm-corpus/{typeorm,nest,payload,remix,svelte}` — **`--fast` first (free), then `--full` on typeorm (18 calls, spends)**. `analyze` writes `.sourcevision/` into the **target** repo, which no worktree isolates and which is shared and un-versioned, so **nobody else should analyze anything under that tree until this row is released.** Also writes `<repo>/.n-dx.json` teacher pins. **Does NOT touch hono or trpc** — they are the evaluation set and the builder asserts on repo identity. **Does NOT `git pull` any clone.** **Adding, per `IMPL-2026-09-18-nutella-…`: `scripts/elm-features.mjs`, `scripts/elm-dataset-build.mjs`, `scripts/elm-coverage-features.mjs`, `scripts/elm-normaliser-sweep.mjs`, and `scripts/data/datasets/**`** — announced before creation per the shared-`scripts/` rule. All analysis-tier, no workspace dependency, no lockfile change. ⛔ **`scripts/elm-coverage-check.mjs` is Jam's and is NOT modified** — it produced the committed 28.0% baseline and a change to it would put that comparison in doubt; the new harness is a sibling that must first **reproduce** its numbers. **Adding `scripts/elm-feature-survey.mjs`** (announced before creation per the shared-`scripts/` rule): read-only structural survey over committed `.sourcevision/` artifacts, no LLM calls, no writes to any analyzed repo — it is the committed, re-runnable form of the measurements behind `ADR-2026-09-17-nutella-elm-training-database-construction.md`. **Gold set #2's 250 blind files are never sampled or trained on.** ⚠️ **AMENDED 2026-09-16 — the 105 UNSAMPLED gold-set-#2 rows are also off limits**, which this row originally implied were fair game. Verified from `k2-goldset2-llm-labels.json` (`poolSize: 355`, `sampled: 250`, provenance = **hono + trpc only**): harvesting them puts our only two fresh-ecosystem probes into the training vocabulary. The existing contamination assertion is path-level and would pass; the contamination is **ecosystem-level**. Caught by Jam. **Also claiming `scripts/data/elm-archetype-corpus-sanity.json`** (`TN-N5`) — 473 algorithmic rows, the inverse population, previously in nobody's plan. ⚠️ **Shared-resource warning to anyone else on this team:** `sourcevision analyze` writes `.sourcevision/` into the **target** repo, and the staging tree `/Users/nolanmoore/Work/n-dx-elm-corpus/` is shared and un-versioned — **say so here before re-running anything against it.** | **Shape decided 2026-09-16: a separate GitHub repo** (needs a second lead before it is created — outward-facing). Released on the first shareable corpus being published into it. |

| 2026-09-16 | Nutella | Nolan | **One-line `ALLOWED` entry in `tests/e2e/architecture-policy.test.js` for `scripts/elm-goldset2-packet.mjs`. Claiming because `tests/e2e/**` is on the shared "nobody edits unilaterally" list.** ⚠️ **This is a pre-existing RED architecture test, not a change I am proposing.** The script imports `execFileSync` from `node:child_process` (`elm-goldset2-packet.mjs:29`) and was committed in `d3da0603` (2026-09-01, K2's `TN-J30`) **without the matching ALLOWED entry**, so `architecture-policy.test.js > no direct child_process imports outside allowed files` has failed on this branch for 15 days. Found while running the suite against my own builder change; **not caused by it** — neither file is modified by me. Exception is legitimate and identical in category to the two entries directly above it: the only use is `execFileSync("git", ["-C", dir, ...])` at `:65` for corpus provenance, and `@n-dx/llm-client` does not resolve from `scripts/`. Watched it go red before fixing. | `tests/e2e/architecture-policy.test.js` — one line plus a comment. | Immediately — single-line edit |
...
- **Team Nolan:** *In flight* — `TN-J32` corpus v2, now **Jam's** (K2 retired 2026-09-04; its open rows absorbed). **Phase 3 certification FAILED and the tier does not generalise:** the v1 model collapses to the majority class on unseen repos (96.4% `service`/`utility` vs the teacher's 48.4%, 5 of 13 labels emitted, coverage 34.9% → 13.2%) — it learned n-dx's archetype prior, not a path→archetype mapping. Caught with **no ground truth and no labelling spend**, so gold set #2's 250 files are still blind and reusable. Fix in progress: corpus v2 (624 rows / 7 ecosystems, was 324 / 2), Phase 1b adopted `elm-4096 tanh` (+2.06 pp, 7 of 9). ⚠️ **v2 is an UNVALIDATED fix — the coverage re-check has not been run on it**, and neither has the corpus-v2 re-freeze (the 4096 fit OOM'd; `69259048` streams it, and the run was never repeated) or the pre-registered Phase 1c capacity arm. **Everything is pushed and the duplicate branch ref is merged (2026-09-04):** local and `origin/Nolan-Work` are both at `76200779`, so the Phase 1c pre-registration — a bar declared before its numbers exist — is now visible to Jarrett and Thomas. The case-duplicated ref (`Nolan-Work` packed at `b0003e7c` vs `nolan-work` loose) is consolidated to a single `Nolan-Work`; it was a fast-forward with 0 unique commits on the stale side, and recovery refs are under `refs/backup/2026-09-04-*`. ⚠️ **The corpus-v2 re-freeze is RUNNING** (`pid 25842`, `nohup`'d, launched by K2 before retirement) — `scripts/data/elm-frozen-model-v2.json` existing is the only success test, and the exit code has already lied once on this job. *Cross-team:* the corpus is documented as a shared asset for anyone's ELM work in [`Nolan-Agents/ELM-CORPUS.md`](Nolan-Agents/ELM-CORPUS.md) — labels are an LLM teacher (72.3% vs truth), not ground truth.
- **Team Jarrett:** <…>
- **Team Thomas:** <…>
