...
| 2026-08-23 | Butter | Nolan | `TN-J3` Lane A4/A6 — **adding `scripts/elm-token-baseline.mjs`**, announced before creation per the shared-`scripts/` rule. Measures per-spawn token overhead across **real classify calls** (30 files of context), at Jam's request — my earlier 22k–46k range came from trivial 2-in/4-out prompts and a classify batch may differ materially. Emits a provenance-carrying fixture, since `.hench/runs/` is gitignored and an uncommitted observation is worthless. **Spends tokens deliberately, bounded to a handful of calls.** | New: `scripts/elm-token-baseline.mjs` + `scripts/data/elm-token-baseline.json`. **Read-only** against `~/n-dx-elm-corpus/AsterMind-Community-Edition/.sourcevision/` — no analyze run, so no claim on that repo needed. Does not touch `scripts/elm-*` files owned by Jam. | Same session |
