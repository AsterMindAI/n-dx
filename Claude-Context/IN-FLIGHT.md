...
| 2026-08-23 | Jam | Nolan | `TN-J13` — **my two analysis scripts fail `architecture-policy.test.js`, so `pnpm test` is red for everyone on the branch.** Found by Butter while validating their own fix, and confirmed not caused by it. `scripts/elm-corpus-build.mjs` has carried it since `8617f9f1`; `scripts/elm-calls-avoided.mjs` (`f91370f8`) added the second. Both import `node:child_process` only to read git provenance (`rev-parse`, `remote.origin.url`). **This claims a shared file** — `tests/e2e/**` is on the list below, because changing it changes everyone's rules. Small, same-day, one-line-per-script edit. | Edits `tests/e2e/architecture-policy.test.js` (adds two entries to `ALLOWED`). No source files, no state-writing commands. | Same day |
